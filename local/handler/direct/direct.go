...
func (tc *directStream) Auth(user string) error {
	return nil
}
...
func (tc *directMuxSession) OpenStream() (proxy.MuxStream, error) {
	tc.streamsMutex.Lock()
	defer tc.streamsMutex.Unlock()
	stream := &directStream{
		conf:    tc.conf,
...
func (p *DirectProxy) CreateMuxSession(server string) (proxy.MuxSession, error) {
	session := &directMuxSession{
		conf:    &p.BaseProxy.Conf,
